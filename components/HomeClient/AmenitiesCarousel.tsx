...
export default function AmenitiesCarousel({ initialData }: { initialData: GalleriesByTab }) {
  ...
  // Embla setup with responsive looping based on visible slides
  const [emblaRef, emblaApi] = useEmblaCarousel({
    loop: items.length > getVisibleSlides(), // Enable loop if we have more slides than visible
    align: "start",
    slidesToScroll: 1,
  ...
  // Improved autoplay with responsive handling of looping
  useEffect(() => {
    const visibleSlides = getVisibleSlides();

    // Only autoplay if we have more slides than what's visible
    if (!emblaApi || !isPlaying || items.length <= visibleSlides) return;

    const id = setInterval(() => {
      if (emblaApi.canScrollNext()) {
        emblaApi.scrollNext();
      } else {
        // Only loop if we have more slides than visible
        if (items.length > visibleSlides) {
          emblaApi.scrollTo(0);
        }
      }
    }, 4000); // 4 second interval for good UX

    return () => clearInterval(id);
  }, [emblaApi, isPlaying, items.length, viewportWidth]); // Added viewportWidth dependency
    ...
    <section id="amenities" className="bg-white relative overflow-hidden">
      ...
      <div className="mx-auto w-full max-w-[1542px] px-3 sm:px-4">
        ...
        <div className="flex flex-col items-center justify-center">
          <h2
            className="font-semibold text-[#10410f] tracking-tight text-center"
            style={{ fontSize: "clamp(22px, 3.2vw, 38px)" }}
          >
            FLOOR PLANS
          </h2>
      ...
      <Lightbox
        ...
        controller={{ closeOnPullDown: true, closeOnBackdropClick: true, disableSwipeNavigation: true }}
      />
    </section>
  );
}
